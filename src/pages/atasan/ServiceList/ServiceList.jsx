...
const ServiceList = ({ onViewDetail }) => {
    ...
    const fetchRequests = async () => {
      ...
      try {
        ...
        const res = await apiRequest(`/service-requests?${query.toString()}`, {
          cacheKey: `service-requests:atasan:${page}:${perPage}:${searchParams.get('search') || ''}:${statusFilter}`,
          staleTime: 60 * 1000,
        });
  ...
  const handleViewDetail = (row) => {
    ...
    navigate(`/service-requests/${row.id}?from=service-requests&variant=${row.detailVariant}`);
  };
